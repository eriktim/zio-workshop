...
object hangman extends App {
  ...
  def gameLoop(state: State): ZIO[Console, IOException, State] =
    for {
      ...
      cont  <- if (state.playerWon) console.putStrLn(s"congrats ${state.name}").const(false) // map to `ZIO.succeed(false)`
               else if (state.playerLost) console.putStrLn("lost, try again").const(false)
               else console.putStrLn("keep going").const(true)
  ...
  def getChoice: ZIO[Console, IOException, Char] =
    console.putStrLn("guess letter") *> console.getStrLn.map(_.toLowerCase.trim.toList).flatMap {
      case letter :: Nil => ZIO.succeed(letter)
      case _ => console.putStrLn("didnt enter") *> getChoice
    }

  def getName: ZIO[Console, IOException, String] =
    console.putStrLn("get name") *> console.getStrLn

  def chooseWord: ZIO[Random, Nothing, String] = random.nextInt(Dictionary.length).map(index => Dictionary.lift(index).getOrElse("defect"))
